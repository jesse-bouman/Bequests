...
def inherit(B, k_sons, k_daughters):
	...
	for i in range(n):
		...
		for k in range(ds):
			i_women[ind+k] = inher
			identifiers_women[ind+k] = k_sons[i] + k_daughters[i]
		ind = ind + ds 
	...
	for i in range(n):
		...
		for k in range(sons):
			i_men[ind+k] = inher
			identifiers_men[ind+k] = k_sons[i] + k_daughters[i]
		ind = ind + sons
...
def main():
	...
	for i in range(m):
		...
		It, k1t = matchBachelors(i_men, i_women, identifiers_men, identifiers_women)
		C_t, B_t, W_t = allocation(It)
	
	print np.sum(decileW(W_t))
	s1 = W_t / np.sum(W_t)
	s1 = np.sort(s1)
	Lorenz = np.zeros(n)
	for i in range(n):
	...
	X = np.linspace(0,1,n)
	plt.xlabel("total number of sisters and brothers")
	plt.ylabel("Wealth")
	print (Es/(1+nu))
	plt.scatter(k1t,W_t)
	#plt.show()
